...
class Arena:
	DIRS = [np.array([-1,0]), np.array([0,1]), np.array([1,0]), np.array([0,-1])]
	def __init__(self, size=(15, 15), num_snakes=1, num_foods=1, num_obstacles=None, walls=False, vector=True, rew_func=None, block_size_limit=None):
		self.die_reward = -1.
		self.move_reward = 0.
		self.eat_reward = 1.
		...
		if num_obstacles is not None:
			if num_obstacles > max(self.size)//3: 
	...
	def register_snake(self):
		...
		for i in range(len(new_snakes)-1):
			x = i
			for j in range(len(new_snakes[x].body)):
				if self.arena[new_snakes[x].body[j][0], new_snakes[x].body[j][1]] == 255:
					del(x)
		...
		new_snake = random.choice(new_snakes)
		...
		if self.vector:
			for snake in self.get_alive_snakes():
				...
				around_head = (self.arena[hl1+1, hl2-1], self.arena[hl1+1, hl2], self.arena[hl1+1, hl2+1],
					...
					self.arena[hl1-1, hl2-1], self.arena[hl1-1, hl2], self.arena[hl1-1, hl2+1])
